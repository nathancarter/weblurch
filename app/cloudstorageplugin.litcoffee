...
# Cloud Storage Plugin for [TinyMCE](http://www.tinymce.com)

This plugin will leverage [the cloud storage tools developed
here](https://lurchmath.github.io/cloud-storage/) to provide load/save
dialogs in TinyMCE that can access the browser's LocalStorage or an online
...
automatically change their app names as well.
        ...
        @setAppName: ( newname = null ) ->
            CloudStorage::appName = newname
            for instance in CloudStorage::instances
                instance.setAppName newname
...
changed with the setters defined in the following section.
        ...
        constructor: ( @editor ) ->
            @setAppName CloudStorage::appName
...
Lastly, keep track of this instance in the class member so that
`@setAppName` can find them later.

            CloudStorage::instances.push this
...
To clear the contents of the document, use this method in the editor's
`CloudStorage` member.  It handles notifying this instance that the document
is then clean. It does *not* check to see if the document needs to be saved
first; it just outright clears the editor.  It also clears the filename, so
that if you create a new document and then save, it does not save over your
last document.
...
the user cancels), then the clear is aborted.  Otherwise, clear is run.
        ...
        tryToClear: =>
            if not @documentDirty
                ...
                @editor.focus()
                return
            # the following code must be updated to use cloud-storage.js
...
is no filename, then a save dialog is shown.
            ...
            # the following code must be updated to use cloud-storage.js
            # all the way to the bottom of this function
            if filename
...
of this plugin are set to be the parameters passed here.
        ...
        load: ( filepath, filename ) =>
            # the following code must be updated to use cloud-storage.js
            # this will require extending cloud-storage.js,
            # as well as making this routine asynchronous
            if filename is null then return
...
default, `@load`.
        ...
        defaultCallback = ( p, f ) => @load p, f
        tryToOpen: ( callback = defaultCallback ) =>
...
argument list.
        ...
        tellDialog: ( args... ) ->
            ...
            for frame in frames
                # the following code must be updated to use cloud-storage.js
                # unless, of course, we end up not needint to call this
                # function any more at all
                if 'filedialog/filedialog.html' is frame.getAttribute 'src'
                    return frame.contentWindow.postMessage args, '*'

## Changing UI handlers

        # we may be able to delete this whole section
...
`CloudStorage` class inside the editor, and points the class at that editor.

    tinymce.PluginManager.add 'cloudstorage', ( editor, url ) ->
        editor.CloudStorage = new CloudStorage editor

## Event handler

When a file dialog sends a message to the page, we look through the existing
instances of the `CloudStorage` class for whichever one wants to handle that
message.  Whichever one has a handler ready, we call that handler.

    # find out where filedialog.html calls this
    # we may be able to simply remove this section now that we're using
    # cloud-storage.js instead
...
Finally, the global function that changes the app name.
    ...
    window.setAppName = ( newname = null ) ->
        CloudStroage::appName = newname
