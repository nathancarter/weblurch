...
(function() {
  ...
  prepareHTML = function(html) {
    var script;
    script = function() {
      var element, install, _i, _len, _ref, _ref1;
      ...
      return (_ref1 = document.getElementsByTagName('input')[0]) != null ? _ref1.focus() : void 0;
    };
  ...
  Dialogs.promptForFile = function(options) {
    var dialog, lastValue, types, value, _ref, _ref1, _ref2, _ref3, _ref4;
    value = options.value ? " value='" + options.value + "'" : '';
    types = options.types ? " accept='" + options.types + "'" : '';
    options.message += "<p><input type='file' " + value + " id='promptInput'/></p>";
    lastValue = null;
    dialog = tinymce.activeEditor.windowManager.open({
      title: (_ref = options.title) != null ? _ref : ' ',
      url: prepareHTML(options.message),
      ...
      buttons: [
        {
          ...
          onclick: function(event) {
            dialog.close();
            return typeof options.cancelCallback === "function" ? options.cancelCallback() : void 0;
          }
        }, {
          type: 'button',
          text: (_ref4 = options.OK) != null ? _ref4 : 'OK',
  ...
  DownloadUpload = (function() {
    function DownloadUpload(editor) {
      var control;
      this.editor = editor;
      ...
      control('upload', {
        ...
        onclick: (function(_this) {
          return function() {
            return _this.uploadDocument();
          };
        })(this)
      });
    }

    DownloadUpload.prototype.downloadDocument = function() {
      var blob, html, link;
      html = embedMetadata(this.editor.getContent(), this.editor.Settings.document.metadata);
      blob = new Blob([html], {
        type: 'text/html'
      });
      link = document.createElement('a');
      link.setAttribute('href', URL.createObjectURL(blob));
      link.setAttribute('download', editor.LoadSave.filename || 'untitled.html');
      link.click();
      return URL.revokeObjectURL(link.getAttribute('href'));
    };

    DownloadUpload.prototype.uploadDocument = function() {
      if (!editor.LoadSave.documentDirty) {
        return this.letUserUpload();
      }
        ...
        buttons: [
          {
            ...
            onclick: (function(_this) {
              return function() {
                editor.LoadSave.tryToSave(function(success) {
                  if (success) {
                    return _this.letUserUpload();
                  }
                });
    ...
    DownloadUpload.prototype.letUserUpload = function() {
      return this.editor.Dialogs.promptForFile({
        title: 'Choose file',
        message: 'Choose an HTML file to upload into the editor.',
        okCallback: (function(_this) {
          return function(fileAsDataURL) {
            var document, html, metadata, _ref;
            html = atob(fileAsDataURL.split(',')[1]);
            _ref = extractMetadata(html), metadata = _ref.metadata, document = _ref.document;
            _this.editor.setContent(document);
            if (metadata != null) {
              _this.editor.Settings.document.metadata = metadata;
            }
  ...
  tinymce.PluginManager.add('downloadupload', function(editor, url) {
    return editor.DownloadUpload = new DownloadUpload(editor);
  });
  ...
  Groups = (function() {
    ...
    Groups.prototype.groupCurrentSelection = function(type) {
      ...
      if (sel.getStart() === sel.getEnd()) {
        ...
        if ((_ref2 = newGroup.parent) != null) {
          _ref2.contentsChanged();
        }
      } else {
        range = sel.getRng();
        ...
        newGroup = this.grouperToGroup(close);
        if ((_ref4 = newGroup.parent) != null) {
          _ref4.contentsChanged();
        }
      }
      return newGroup;
    };
    ...
    Groups.prototype.drawGroups = function(canvas, context) {
      var bodyStyle, bottomEdge, c, connection, connections, drawArrow, drawGroup, extra, from, g, gap, group, groupEdgesToConnect, index, innermost, interp, leftMar, moveBy, numArrays, old, pad, padStep, radius, rightMar, size, tag, tags, tagsToDraw, to, topEdge, walk, x1, x2, y1, y2, _base, _base1, _base2, _base3, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _m, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7, _ref8, _results;
      this.bubbleTags = [];
      if (this.scanLocks > 0) {
        return;
      }
      ...
      drawGroup = (function(_this) {
        return function(group, drawOutline, drawInterior, withTag) {
          ...
          if (drawOutline) {
            ...
            if (type != null) {
              if (typeof type.setOutlineStyle === "function") {
          ...
          if (drawInterior) {
            ...
            if (type != null) {
              if (typeof type.setFillStyle === "function") {
      ...
      _ref1 = (_ref = typeof this.visibleGroups === "function" ? this.visibleGroups() : void 0) != null ? _ref : [];
      for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
        extra = _ref1[_i];
        drawGroup(extra, true, false, true);
      }
      tagsToDraw = [];
      while (tags.length > 0) {
        tag = tags.shift();
      ...
      for (_k = 0, _len2 = tagsToDraw.length; _k < _len2; _k++) {
        ...
        if ((_ref3 = tag.group) != null) {
          if (typeof _ref3.type === "function") {
            if (typeof (_base = _ref3.type()).setFillStyle === "function") {
              _base.setFillStyle(tag.group, context);
            }
          }
        }
        context.fill();
        ...
        if ((_ref4 = tag.group) != null) {
          if (typeof _ref4.type === "function") {
            if (typeof (_base1 = _ref4.type()).setOutlineStyle === "function") {
              _base1.setOutlineStyle(tag.group, context);
            }
          }
        }
        context.stroke();
        ...
        if ((_ref5 = tag.group) != null) {
          if (typeof _ref5.type === "function") {
            if (typeof (_base2 = _ref5.type()).setFillStyle === "function") {
              _base2.setFillStyle(tag.group, context);
            }
          }
        }
        context.fill();
        ...
        if (!context.drawHTML(tag.content, tag.x1 + padStep, tag.y1, tag.style)) {
          setTimeout(((function(_this) {
            return function() {
              var _ref6;
              return (_ref6 = _this.editor.Overlay) != null ? _ref6.redrawContents() : void 0;
            };
          })(this)), 10);
          return;
        }
      ...
      drawArrow = (function(_this) {
        return function(index, outOf, from, to, label, setStyle) {
          var centerX, centerY, endX, endY, fromBox, how, startX, startY, style, toBox, _ref6, _ref7, _ref8;
          context.save();
          context.strokeStyle = ((_ref6 = from.type()) != null ? _ref6.color : void 0) || '#444444';
          context.globalAlpha = 1.0;
          context.lineWidth = 2;
          ...
          if (label !== '') {
            ...
            if (!(size = context.measureHTML(label, style))) {
              setTimeout((function() {
                var _ref7;
                return (_ref7 = _this.editor.Overlay) != null ? _ref7.redrawContents() : void 0;
              }), 10);
              return;
            }
            ...
            context.fill();
            context.lineWidth = 1.5;
            context.strokeStyle = (_ref7 = (_ref8 = from.type()) != null ? _ref8.color : void 0) != null ? _ref7 : '#444444';
            context.stroke();
            context.fillStyle = '#000000';
            context.globalAlpha = 1.0;
      ...
      _ref7 = [group].concat(__slice.call((_ref6 = typeof this.visibleGroups === "function" ? this.visibleGroups() : void 0) != null ? _ref6 : []));
      _results = [];
      for (_l = 0, _len3 = _ref7.length; _l < _len3; _l++) {
        g = _ref7[_l];
        if (g) {
          connections = typeof (_base3 = g.type()).connections === "function" ? _base3.connections(g) : void 0;
          numArrays = ((function() {
            var _len4, _m, _results1;
            _results1 = [];
            for (_m = 0, _len4 = connections.length; _m < _len4; _m++) {
              c = connections[_m];
          ...
          _ref8 = connections != null ? connections : [];
          for (_m = 0, _len4 = _ref8.length; _m < _len4; _m++) {
            ...
            if (!(connection instanceof Array)) {
              ...
              drawGroup(connection, true, false, false);
            }
          }
          _results.push((function() {
            var _len5, _n, _ref9, _results1;
            _ref9 = connections != null ? connections : [];
            _results1 = [];
            for (index = _n = 0, _len5 = _ref9.length; _n < _len5; index = ++_n) {
              connection = _ref9[index];
              if (connection instanceof Array) {
                from = typeof connection[0] === 'number' ? this[connection[0]] : connection[0];
                to = typeof connection[1] === 'number' ? this[connection[1]] : connection[1];
                _results1.push(drawArrow.apply(null, [index, numArrays, from, to].concat(__slice.call(connection.slice(2)))));
              } else {
  ...
  keyboardShortcutsWorkaround = function(editor) {
    var createShortcutData, maybeInstall, shortcuts;
    shortcuts = [];
    createShortcutData = function(text) {
      var key, modifiers, _i, _ref;
      _ref = text.toLowerCase().replace(/\s+/g, '').split('+'), modifiers = 2 <= _ref.length ? __slice.call(_ref, 0, _i = _ref.length - 1) : (_i = 0, []), key = _ref[_i++];
      return {
        altKey: __indexOf.call(modifiers, 'alt') >= 0,
        ctrlKey: __indexOf.call(modifiers, 'ctrl') >= 0,
        metaKey: __indexOf.call(modifiers, 'meta') >= 0,
        key: key
      };
    };
    ...
    editor.addMenuItem = function(name, settings) {
      maybeInstall(name, settings);
      return editor.__addMenuItem(name, settings);
    };
    editor.__addButton = editor.addButton;
    editor.addButton = function(name, settings) {
      maybeInstall(name, settings);
      return editor.__addButton(name, settings);
    };
    return editor.on('keydown', function(event) {
      var key, matches, shortcut, value, _i, _len, _ref;
      for (_i = 0, _len = shortcuts.length; _i < _len; _i++) {
        shortcut = shortcuts[_i];
        matches = true;
        _ref = shortcut.keys;
        for (key in _ref) {
  ...
  $(function() {
    ...
    return tinymce.init({
      ...
      menu: {
        file: {
          title: 'File',
          items: 'newfile openfile | savefile saveas download upload | managefiles | print' + moreMenuItems('file')
        },
        edit: {
          title: 'Edit',
