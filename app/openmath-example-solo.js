...
(function() {
  ...
  window.helpAboutText = '<p>See the fully documented <a target="top" href="https://github.com/nathancarter/weblurch/blob/master/app/openmath-example-solo.litcoffee" >source code for this demo app</a>.</p>';
  ...
  window.groupTypes = [
    {
      ...
      tagContents: function(group) {
        return window.getTagExternalName(group.get('tagName'));
      },
      tagMenuItems: function(group) {
        return window.XMLMenuItems(group);
      },
  ...
  window.setTagData({
    ...
    CDComment: {
      ...
      documentation: '<p>This tag will not actually be used in authoring, but will be automatically wrapped around any top-level text in the document when XML is generated.</p>'
    },
    CDName: {
      externalName: 'Content Dictionary Name',
      documentation: '<p>The name of the Content Dictionary.  This will be the "cd" field of all symbols from this Content Dictionary.  For example, standard addition comes from the Content Dictionary "arith1".  It must comply to the rules in Section 2.3 of <a href="http://www.openmath.org/standard/om20-2004-06-30/omstd20.pdf">the OpenMath Standard version 2.0</a>.</p> <p>This element is required in a Content Dictionary, and must either be the first element or immediately after the optional description.</p> <p>Source: <a href="http://www.openmath.org/standard/om20-2004-06-30/omstd20.pdf">the OpenMath Standard version 2.0</a></p>',
      belongsIn: ['CD'],
      belongsAfter: ['Description', null],
      unique: true,
      contentCheck: function(group) {
        var tryToMakeVariable;
        tryToMakeVariable = OM.variable(group.contentAsText().trim());
        if (!(tryToMakeVariable instanceof OMNode)) {
          return ['Content Dictionary Names must be valid identifiers. The content of this element is not.'];
        } else {
          return [];
        }
      }
    ...
    CDURL: {
      ...
      contentCheck: function(group) {
        var re;
        re = /^(https?|ftp):\/\/[^\s/$.?#].[^\s]*$/i;
        if (!re.test(group.contentAsText().trim())) {
          return ['Content Dictionary URL elements must contain valid URLs.  This one does not.'];
        } else {
          return [];
        }
      }
    ...
    CDUses: {
      ...
      contentCheck: function(group) {
        ...
        if (notOkay.length > 0) {
          return ["Content Dictionary Uses must be lists of valid identifiers.  The following words are not valid identifiers:  " + (notOkay.join(', '))];
        } else {
          return [];
        }
      }
    ...
    CDReviewDate: {
      ...
      documentation: '<p>A review date is a date until which the content dictionary is guaranteed to remain unchanged.  Dates should be written in the ISO-compliant format YYYY-MM-DD, e.g. 1966-02-03.</p> <p>This element is optional.  If present, there can be only one, and it must follow the Content Dictionary Name and optional URL and Base URL.</p> <p>Source: <a href="http://www.openmath.org/standard/om20-2004-06-30/omstd20.pdf">the OpenMath Standard version 2.0</a></p>',
      belongsIn: ['CD'],
      belongsAfter: ['Name', 'CDURL', 'CDBase'],
      unique: true,
      contentCheck: function(group) {
        var date, match, month;
        match = /^([0-9]{4})-([0-9]{2})-([0-9]{2})$/.exec(group.contentAsText().trim());
        if (!match) {
          return ['This element must contain a valid date of the form YYYY-MM-DD.'];
        } else {
          month = parseInt(match[2]);
          date = parseInt(match[3]);
          if (month < 1 || month > 12) {
            return ["This element must contain a valid date in the form YYYY-MM-DD.  The month " + month + " is invalid."];
          } else if (date < 1 || date > 31) {
            return ["This element must contain a valid date in the form YYYY-MM-DD.  The date " + date + " is invalid."];
          } else {
            return [];
          }
        }
    ...
    CDDate: {
      ...
      unique: true,
      contentCheck: function(group) {
        return window.getTagData('CDReviewDate', 'contentCheck')(group);
      }
    },
    CDVersion: {
      ...
      contentCheck: function(group) {
        if (/[0-9]+/.test(group.contentAsText().trim())) {
          ...
          return ["A Content Dictionary Version must be a nonnegative integer."];
        }
      }
    },
    CDRevision: {
      ...
      contentCheck: function(group) {
        if (/[0-9]+/.test(group.contentAsText().trim())) {
          ...
          return ["A Content Dictionary Revision must be a nonnegative integer."];
        }
      }
    },
    CDDefinition: {
    ...
    Name: {
      ...
      contentCheck: function(group) {
        ...
        tryToMakeVariable = OM.variable(group.contentAsText().trim());
        if (!(tryToMakeVariable instanceof OMNode)) {
          return ['Symbol Names must be valid identifiers. The content of this element is not.'];
        } else {
          return [];
        }
      }
    ...
    MathematicalProperty: {
      ...
      defaultChild: 'OMOBJ',
      documentation: '<p>A Mathematical Property can come in one of two forms.</p> <ul> <li><b>Formal:</b> In this form, the content of the Mathematical Property should be just a single OpenMath Object.  Thus OpenMath is the language used to formalize Mathematical Properties in Content Dictionaries.</li> <li><b>Commented:</b> In this form, the content of the Mathematical Property can be any text, and can express a Mathematical Property in whatever informal language or description seems appropriate.</li> </ul> <p>Source: <a href="http://www.openmath.org/standard/om20-2004-06-30/omstd20.pdf">the OpenMath Standard version 2.0</a></p>',
      belongsIn: ['CDDefinition'],
      alterXML: function(XML, group) {
        if (XML.indexOf('<OMOBJ>') > -1) {
          return XML.replace(/MathematicalProperty>/g, 'FMP>');
        } else {
          return XML.replace(/MathematicalProperty>/g, 'CMP>');
        }
      },
      allowedChildren: {
        OMOBJ: [0, 1]
...
//# sourceMappingURL=openmath-example-solo.js.map
