 ...
 * Attributes set in a group with `Group::set` must be identifiers, all
   lower case, but other than that can be (just about) anything.  These are
   not in any way manipulated, encoded, or changed before storing
   interanlly as `data-*` attributes.
 * Attributes set by the document author, then hidden in a group (or just
   created hidden in the first place) are encoded using
   `OM.encodeAsIdentifier` before being passed to `Group::set`.  These
   always come out in the form `id_X`, where the `X` is a sequence of
   decimal digits.  Thus callers of `Group::set` should avoid this (unusual)
   form in their own keys.  These must also be valid identifiers, even
   before conversion into that form, for the reason given in the following
   bullet point.
 * When converting groups to complete form, keys not of the form `id_X` are
   ignored, because they are for internal Lurch use only, and never known or
   seen by the user.  Keys of the form `id_X` are converted back to their
   original form using `OM.decodeIdentifier` before then being converted
   into OpenMath symbols (using `OM.sym orig, 'Lurch'`) for use as
   attribute keys of the OpenMath tree generated by `completeForm()`.  Thus
   the original key for a group attribute in the document must be a valid
   OpenMath identifier.
